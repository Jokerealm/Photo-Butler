...
const PromptEditor: React.FC<PromptEditorProps> = ({
  ...
  // Check if prompt is empty (for button state)
  const isPromptEmpty = prompt.trim().length === 0;

  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-semibold">3. 编辑提示词</h2>
        {selectedTemplate && (
          <button
            onClick={resetPrompt}
            disabled={disabled}
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          <textarea
            ...
            className={`
              w-full min-h-[120px] p-4 border rounded-lg resize-y
        ...
        <div className="space-y-2">
          ...
          {!selectedTemplate && (
            <div className="flex items-center space-x-2 text-gray-500">
              ...
              <span className="text-sm">请先选择一个模板以加载默认提示词</span>
            </div>
          )}
          ...
          {prompt.trim().length > 0 && !disabled && (
            <div className="flex items-center space-x-2 text-green-600">
              ...
              <span className="text-sm">提示词已准备就绪，可以开始生成图片</span>
            </div>
          )}
        </div>
        ...
        <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <h4 className="text-sm font-medium text-gray-900 mb-2">提示词编辑技巧:</h4>
